...
`)
  ...
  if (editorTypeSql && !editorTypeSql.includes("'ckeditor'")) {
    ...
    INSERT INTO posts_new (
      id, title, slug, category_id, author_id, visibility, status, editor_type, content, created_at, updated_at, deleted_at
    )
    SELECT
      id, title, slug, category_id, author_id, visibility,
      COALESCE(status, 'published'),
      editor_type, content, created_at, updated_at, deleted_at
    FROM posts;
